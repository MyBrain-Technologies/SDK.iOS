...
// Update on 23/03/2017 by Fanny Grosselin (Change float by double for the functions not directly used by Androïd. For the others, keep inputs and outputs in float, but do the steps with double or create two functions : one with only float, another one with only double)
...
vector<float> BandPassFilter(vector<float> tmp_RawSignal, vector<float> tmp_freqBounds)
{
    ...
    {
        tmp_H.push_back(std::complex<float>(H[ki], 0));
    }
    int HSize = H.size();
    ...
    {
        tmp_Mirrored.push_back(std::complex<float>(Mirrored[ki], 0));
    }
